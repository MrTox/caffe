...
void ComplexPoolingLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void ComplexPoolingLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  CHECK_EQ(5, bottom[0]->num_axes()) << "Input must have 5 axes, "
      << "corresponding to (num, channels, height, width, complex)";
...
// case?
template <typename Dtype>
void ComplexPoolingLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  case PoolingParameter_PoolMethod_MAX:
  {
    ...
    if (use_top_mask) {
      ...
      caffe_set(top_count, std::complex<Dtype>(-1), top_mask);
    } else {
...
template <typename Dtype>
void ComplexPoolingLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  caffe_set(bottom[0]->count()/2, std::complex<Dtype>(0), bottom_diff);
  // We'll output the mask to top[1] if it's of size >1.
  const bool use_top_mask = top.size() > 1;
  ...
  case PoolingParameter_PoolMethod_MAX:
    // The main loop
    if (use_top_mask) {
      top_mask = this->RealToComplexTopData_cpu(top, 1);
    } else {
        ...
        for (int ph = 0; ph < pooled_height_; ++ph) {
          for (int pw = 0; pw < pooled_width_; ++pw) {
            ...
            const int bottom_index =
                use_top_mask ? std::real(top_mask[index]) : mask[index];
      ...
      for (int c = 0; c < channels_; ++c) {
        for (int ph = 0; ph < pooled_height_; ++ph) {
          for (int pw = 0; pw < pooled_width_; ++pw) {
            ...
            int hend = min(hstart + kernel_h_, height_ + pad_h_);
            int wend = min(wstart + kernel_w_, width_ + pad_w_);
            std::complex<Dtype> pool_size( (hend - hstart) * (wend - wstart) );
            hstart = max(hstart, 0);
            wstart = max(wstart, 0);
            hend = min(hend, height_);
            wend = min(wend, width_);
