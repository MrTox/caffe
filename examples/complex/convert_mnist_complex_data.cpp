...
void convert_dataset(const char* image_filename, const char* label_filename,
  ...
  std::complex<char>* c_pixels = new std::complex<char>[rows * cols];
  int count = 0;
  string value;
  ...
  for (int item_id = 0; item_id < num_items; ++item_id) {
    ...
    for (int i = 0; i < rows*cols; i++) {
      c_pixels[i] = std::complex<char>(pixels[i], 0);
    }

    datum.set_data(reinterpret_cast<char*>(c_pixels), rows*cols*2);
