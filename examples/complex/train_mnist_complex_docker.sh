...
if [ ${CREATE_DB:-1} -eq 1 ]
then
$DOCKER_RUN bash -c "mkdir -p ./examples/complex;
                     cp -ru \$CAFFE_ROOT/examples/complex/create_mnist_complex.sh ./examples/complex/;
                     sed -i s#BUILD=build#BUILD=\$CAFFE_ROOT/build## ./examples/complex/create_mnist_complex.sh"
$DOCKER_RUN ./examples/complex/create_mnist_complex.sh
fi

# Train the network
if [ ${TRAIN:-1} -eq 1 ]
then
$DOCKER_RUN bash -c "cp \$CAFFE_ROOT/examples/complex/mnist_complex_solver.prototxt ./examples/complex/;
                     cp \$CAFFE_ROOT/examples/complex/mnist_complex_train_test.prototxt ./examples/complex/"
    # Ensure that the solver_mode is compatible with the desired GPU mode.
    if [ ${GPU:-0} -eq 0 ]
    then
    $DOCKER_RUN sed -i 's#solver_mode: GPU#solver_mode: CPU##' ./examples/complex/mnist_complex_solver.prototxt
    fi
$DOCKER_RUN caffe train --solver=examples/complex/mnist_complex_solver.prototxt $*
fi
